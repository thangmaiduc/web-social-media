...
export default function SearchFeed({ textSearch }) {
  ...
  let usersQuery = useQuery(api.QUERY_USERS, pageUser, { textSearch },);
  let postsQuery = useQuery(api.QUERY_POSTS, pagePost, { textSearch },);
    ...
    <div className='feed'>
      <div className='feedWrapper'>
        <div className="userWrapepr">
          ...
          {
            lengthUser === 0 ?
              <p>{`Không có người dùng nào có từ khóa '${textSearch}'`}</p> :
        ...
        <div className="postWrapepr">
          ...
          {
            lengthPost === 0 ?
              <p>{`Không có bài viết nào có từ khóa '${textSearch}'`}</p> :
